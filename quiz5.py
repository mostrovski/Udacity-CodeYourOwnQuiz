...
My code starts with four imports. 'sys' is a result of my intention to stop the 
execution of the code when a user runs out of attempts giving no correct answer.
Examining the sample provided by Udacity, I noticed that it closes the window in
such a situation. I wanted to avoid it somehow, and thus googled a bit and found
the 'import sys + sys.exit()' solution that worked just how I wanted in the
Git Bash. The second import - 'textwrap' - was added at the very end of the 
project when I decided to do something with ugly looking output paragraphs. Just
like with 'sys', I googled and found the 'import textwrap + textwrap.fill()' 
solution. Next, 'quiz_data' imports my file with the data used in the quiz. 
Finally, I decided to import 'randint' to diversify some prompts. 
'''
import sys
import textwrap
import quiz_data
...
output_line_len = 75 
'''
Sets the lenght of the output line; used by textwrap.fill in the 'quiz_game' 
function below.
'''
...
def input_check(s,options):
	'''
	Takes two arguments as input: the string 's' and the list 'options'. 
	Returns True if the string is in the list. Otherwise, returns False. It is 
	used to analyze the user's input during the game.   
	'''
	return s in options
...
def level_input():
	''' 
	Has no parameters. Finds out which level the user prefers and returns it.
	Includes the welcome line:
	'''
	current_level = level_chosen(raw_input(quiz_data.welcome),level_options)
	while level_chosen(current_level,level_options) == None: 
		current_level = level_chosen(raw_input(quiz_data.wrong_level),\
					        level_options)
	'''
...
def attempts_input():
	'''	
	Has no parameters. Finds out how many attempts per problem the user wants 
	to have and returns it. User's input is saved as integer to be 	usable later
	in the 'quiz_problem' function: 
	'''
...
def random_correct():
	'''
	Has no parameters. Uses the randint function (must return either 1 or 2 or 
	3) to return one of three possible strings randomly. It is used to diversify 
...
def quiz_problem(problem,correct_answer,attempts):
	'''
	The heart of the code! 
	This function manages every separate problem of the quiz. Takes three 
	arguments as input: the string 'problem', the list 'correct_answer', and 
	the integer 'attempts'. Checks if the user gave the correct answer using no 
	more attempts than it was allowed. In this case, it returns the correct 
	answer of the user ('user_input'). Otherwise, it stops the execution of the 
	code ('sys.exit'). Here is how:
	'''
...
def quiz_game(level,quiz_blanks_level,correct_answers_level,attempts):
	'''
	This function manages the game using the variables and functions defined 
	above. It takes four arguments as input: the string 'level', the list 
	'quiz_blanks_level', the list 'correct_answers_level', and the number of 
	attempts. They are to be replaced with '..._paragraph', '..._blanks',  
	'..._answers' variables (see variables in the file quiz_data), and the 
	integer depending on the level and the number of attempts the user chooses 
	during the game. It doesn't return anything itself, it is basically used to 
	call other functions in the right order. Here is the logic:     
	'''
	index = 0 
	'''
	...
	# goes through all the blanks in the quiz_blanks list.
		...
		correct: 
		'''
		answer = quiz_problem(problem,correct_answers_level[index],attempts) 
...
def quiz_game_start(current_level,attempts):
	'''
	Initiates the input of the level and attempts per problem the user wants to 
	have.

	Depending of the value of the 'current_level' and 'attempts' the 'guiz_game'
	gets relevant arguments. The user is notified of the level and the number of 
	attempts per problem chosen, whereafter the game starts:
	'''
	...
	if current_level == 'easy':	
		quiz_game(quiz_data.easy_paragraph,quiz_data.easy_blanks,\
			quiz_data.easy_answers,attempts)
	elif current_level == 'medium':
		quiz_game(quiz_data.medium_paragraph,quiz_data.medium_blanks,\
			quiz_data.medium_answers,attempts)
	else:
		quiz_game(quiz_data.hard_paragraph,quiz_data.hard_blanks,\
			quiz_data.hard_answers,attempts)

quiz_game_start(level_input(),attempts_input()) # Let the Game Begin :)
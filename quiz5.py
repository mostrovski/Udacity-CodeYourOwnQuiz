...
def level_input():
	''' 
	Takes no arguments. Finds out which level the user prefers and returns it.
...
def attempts_input():
	'''	
	Takes no arguments. Finds out how many attempts per problem the user wants 
...
def random_correct():
	'''
	Takes no arguments. Uses the randint function (must return either 1 or 2 or 
...
def quiz_problem(problem,correct_answer,attempts):
	...
	arguments as input: the string 'problem', the list 'correct_answer', and the
	integer 'attempts'. Checks if the user gave the correct answer using no more
	attempts than it was allowed. In this case, it returns the correct answer of
	the user ('user_input'). Otherwise, it stops the execution of the code 
	('sys.exit'). Here is how:
	'''
